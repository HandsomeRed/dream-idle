...
test.describe('梦幻放置 - 角色创建流程', () => {
  test('完整的角色创建流程', async ({ page }) => {
    ...
    await expect(page.getByText('梦幻西游题材放置挂机游戏')).toBeVisible();
    ...
    // 8. 验证去战斗按钮存在
    await expect(page.getByTestId('battle-btn')).toBeVisible();
  });
  ...
  test('返回按钮功能', async ({ page }) => {
    ...
    // 验证回到名字输入界面
    await expect(page.getByText('请输入角色名字：')).toBeVisible();
  });
